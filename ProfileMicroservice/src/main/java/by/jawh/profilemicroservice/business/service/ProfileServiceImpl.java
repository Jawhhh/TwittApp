...
public class ProfileServiceImpl implements ProfileService {
    ...
    public String uploadAvatar(Long id, MultipartFile avatar) throws Exception {
        Optional<ProfileEntity> maybeProfile = profileRepository.findById(id);
        if (maybeProfile.isEmpty()) {
            throw new ProfileNotFoundException("profile with id: %s not found :(".formatted(id));
        }

        ProfileEntity profileEntity = maybeProfile.get();
